...
namespace WordSkills.Client.Core.Components;
public partial class AuthenticationStateLogger : AppComponentBase
{
    ...
    private async void LogAuthenticationState(Task<AuthenticationState> task)
    {
        try
        {
            ...
            var (userId, userName, email, isUserAuthenticated) = user.IsAuthenticated() ? (user.GetUserId(), user.GetUserName(), user.GetEmail(), user.IsAuthenticated()) : default;
